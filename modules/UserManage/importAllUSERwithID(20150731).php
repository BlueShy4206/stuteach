      ...
      //debug_msg("第".__LINE__."行 InputArray ", $InputArray);
		...
		for($j=0;$j<$jj;$j++){
            ...
            for($i=0;$i<=$MaxArrayKey;$i++){
				...
				$query = 'INSERT INTO user_info (user_id, uname, email, sex, user_regdate, birthday, organization_id, pass, viewpass, city_code, grade, class, identity, tel, mobil, address) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
				$data = array($user_id, $chkName, $ROW[2], $ROW[3], $user_regdate, $birthday, $_REQUEST['organization'][1], md5($pass), $pass_compiler, $_REQUEST['organization'][0], $_REQUEST['organization'][2], $_REQUEST['organization'][3], $ROW[5], $ROW[6], $ROW[7], $ROW[8]);

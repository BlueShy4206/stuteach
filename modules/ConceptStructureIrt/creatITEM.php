...
function creatITEM($opt, $pid=0, $sid=0){
   ...
   //--  檢查是否為檔案上傳狀態，並回報
	if ($form->validate() && $_REQUEST['opt']=='creat_item' && $cs_id!="000000000") {
      ...
      //die();
		...
		if (PEAR::isError($result)) {
			die('發生錯誤：'.__LINE__.'，'.$result->getMessage());
		}
		...
		$data = array($item_snn, $cs_id, $_REQUEST['parameter_a'], $_REQUEST['parameter_b'], $_REQUEST['parameter_c'], $temp, $temp ,$temp1, $_REQUEST['parameter_dim'], '1');
		//debug_msg("第".__LINE__."行 data ", $data);
		//debug_msg("第".__LINE__."行 _REQUEST ", $_REQUEST);

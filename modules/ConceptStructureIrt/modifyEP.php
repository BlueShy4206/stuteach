...
//debug_msg("第".__LINE__."行 _REQUEST ", $_REQUEST);
if($_REQUEST['opt']=="listEP" || $_REQUEST['opt']=="checkEP" || $_REQUEST['opt']=="modifyEP"){
...
function modifyEP($opt){
	...
	$sql = "select * from exam_paper ORDER BY cs_id, paper_vol";
	/*
	...
	*/
	...
	while ($row=$result->fetchRow()){
        ...
        $CsAry=explode_cs_id($row['cs_id']);
        $row['publisher_id']=$CsAry[0];
        $row['subject_id']=$CsAry[1];
        $row['vol']=$CsAry[2];
        $row['unit']=$CsAry[3];
        $row['concept']=id2csname($row['cs_id']);
		//檢查是否具有該單元的操作權限
        if(in_array( $q_cs_ida, $OpenedCS)==TRUE){
			...
			$select5[$row['publisher_id']][$row['subject_id']][$row['vol']][$row['unit']][$row['paper_vol']]="卷".$row['paper_vol'];
			/*
	...
	$form->addElement('hidden','opt2',$chosen_opt);
...
function listEP($opt2){
	...
	if($opt2=="modifyEP"){
	   $delEP_url="modules.php?op=modload&name=".$module_name."&file=modifyEP&opt=deleteEP&cs_id=".$cs_id.'&paper_vol='.$paper_vol;
	...
	echo "<tr><td align=\"center\" colspan=\"2\"><font class=\"title\"><b>試卷名稱：".$EP_title."</b></font>&nbsp;&nbsp;&nbsp;&nbsp;【".$delEP."】</td>";
	echo '</tr><tr>';
	...
	while ($data = $result->fetchRow()) {
		...
		$sol_pieces=$item_data->sol_pieces;  //詳解的原始檔名
			//debug_msg("第".__LINE__."行 data ", $item_snn);  
		//debug_msg("第".__LINE__."行 item ", $item);
      //debug_msg("第".__LINE__."行 SolItem ", $SolItem);
   //die();
		echo "<tr>";
		echo '<td align="left"><b>&nbsp;&nbsp;【'.$item_num.'】&nbsp;<img src="'.$item['0'].'" align="top"><br><br>';
		for($i=1;$i<=4;$i++){
			for($j=0;$j<10;$j++){   //控制縮排
				echo '&nbsp;';
			}
        ...
        if($opt2=="modifyEP"){
    		$base='modules.php?op=modload&name='.$module_name.'&file=modifyITEM&pid='.$_REQUEST['unit_item'][0].'&sid='.$_REQUEST['unit_item'][1].'&vid='.$_REQUEST['unit_item'][2].'&uid='.$_REQUEST['unit_item'][3].'&paper='.$paper_vol.'&ShowSol='.$ShowSol;
    	...
    	$sql= "select a, b, c, dim  from concept_item_parameter where item_sn ='".$item_snn."' ";
		...
		echo '<td>答案：'.$item['ans'].'<br><br>配分：'.$item['points'].'<br><br>鑑別度︰'.$P_a.'<br><br>難度︰'.$P_b.'<br><br>猜測度︰'.$P_c.'<br><br>向度︰'.$P_dim.'</b></font><br /><br />';
		echo '&nbsp;'.$modify.'<br />&nbsp;'.$del.$ShowItemEduPara.'</td></tr>';
	}
...
function deleteEP($opt){
	...
	if($result==1){
		...
		$RedirectTo="Location: modules.php?op=modload&name=".$module_name."&file=modifyEP&del_ep_id=".$ep_id;
		Header($RedirectTo);
	}else{
		echo "刪除失敗！ 請洽系統管理員！";
	}

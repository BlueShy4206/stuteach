...
function listAllExams_row($user_id){
	...
	echo '<font class="title"><b>'.$user_id.'【'.$q_uname.'】歷來測驗單元</b></font><br>';
	//echo $_REQUEST['q_user_id'];
	echo '<table width="95%" border="1" cellpadding="1" cellspacing="1" bordercolor="#0000FF">';
	//echo '<tr><td align="center"><font class="title"><b>單元名稱</b></font></td></tr>';
...
function listAllExams_col($user_id){  //學生用
   ...
   echo '</td></tr>';
	//echo '<tr><td bgcolor="#46AFE4"><b><font size="4">　'.$user_id.'【'.$q_uname.'】　歷來測驗單元</font></b></td></tr></table>';
	echo '<tr></tr></table>';
	...
	echo '<table width="100%" border="1" cellspacing="2" cellpadding="1">';
          /*echo '<tr>
                <td bgcolor="#D3E3EF" >編號</td>
                <td bgcolor="#D3E3EF" align="center" scope="col">單元名稱</td>
                <td bgcolor="#D3E3EF" align="center" scope="col">能力值</td>
  ...
  //$sql="select * from exam_record_irt WHERE user_id='.$user_id.'";
	//$result =$dbh->query($sql);
	$i=1;
	$sql=mysql_query("select * from exam_record_irt WHERE user_id='".$user_id."'");
  ...
  mysql_free_result($sql);*/
    ...
    echo $cs_title;*/
		...
		//echo '<tr>';
		//echo '<td bgcolor="#ecf8d2" align="center">'.$i.'</td>';
		//echo '<td bgcolor="#ecf8d2" align="left">'.$url.$cs_title.'</a></td>';
		//echo '<td bgcolor="#ecf8d2" align="center">'.$row[theta].'</td>';
...
*/
			$prt[4].=$a->print_radar_gif("example2.1.php", $report_for_pc);
			$prt[4].=$a->print_remedy_data($q_user_id, $q_cs_id, $report_for_pc);   //概念診斷報告
/*
         if($a->score[$a->test_times-1]!=100){  //考100則不必顯示

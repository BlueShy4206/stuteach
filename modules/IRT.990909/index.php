	...
	if(count($myC)>0){
		foreach($myC as $key => $MyCourseID){
			...
			while ($data = $result->fetchRow()) {
				...
				$open_EP[]=$data['course_id']._SPLIT_SYMBOL.$data['exam_paper_id'].$my_type_id;   //已開放之試卷
			}
		}
	}
...
//debug_msg("第".__LINE__."行 open_EP ", $open_EP);
	if(count($open_EP)==0){
		...
		for($j=0;$j<$jj;$j++){
			...
			$select1[$cid][$pid]=id2publisher($pid);
		...
		$sel->setOptions(array($select0, $select1, $select2, $select3, $select4, $select5));

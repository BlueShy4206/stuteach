...
//...................................................................最大訊息法
  ...
  if($select_num < 24)
	  ...
	  $sql=mysql_query("SELECT `item_sn`,`a`,`b`,`c`,`sub` FROM `concept_item_parameter`
	                    ...
	                    and `cs_id` like '".$CsID."' and ( sub!=8 and sub !=7 )");
    }
  elseif($select_num >= 24 && $select_num < 28 )
	  ...
	  $sql=mysql_query("SELECT `item_sn`,`a`,`b`,`c`,`sub` FROM `concept_item_parameter`
	                    WHERE `item_sn` not in ('".$select_item_id_S."') 
	                    and `item_sn` not in ('".$select_testletitem_id_S."')
	                    and `cs_id` like '".$CsID."' and ( sub =8 or sub =7 )");				  
	}
  $max_information = 0;      
	...
	if(isset($testlet)) 
		{
    	 ...
    	 for($i=1;$i<=$n;$i++)
    	 	{
			 $sql=mysql_query("SELECT sub FROM concept_item_parameter WHERE item_sn = '$testlet_item_sn[$n]' ");
	    	 while($row=mysql_fetch_array($sql))
	  			{ 
				 $Sub_items[$row['sub']-1]++;
	      	 	  //debug_msg("第".__LINE__."行 data ", $testlet_item_sn);

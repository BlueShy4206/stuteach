...
echo '<br>
	   <table width="95%" border="1" cellpadding="0" cellspacing="0">
        ...
        //預設值（刪除或新增的功能選錯）
        $RedirectTo="Location: modules.php?op=modload&name=".$module_name."&file=ctrlEP_talent";
	      Header($RedirectTo);
        }
...
//能力指標結構
	if ($file1->isValid()) {
      ...
      //debug_msg("第".__LINE__."行 InputArray ", $InputArray);
		...
		$sql = "delete from concept_item_talent WHERE exam_paper_id = '$ep_id'";
		$result =$dbh->query($sql);	
			 ...
			 //  $MaxArrayKey=max(array_keys($InputArray[$j]));
	            ...
	            for($i=0;$i<=$x;$i++){
					...
					//	echo $ROW[$i];
						if ($ROW[$i]==1)
							{
					 		$query = 'INSERT INTO concept_item_talent ( item_sn, exam_paper_id, talent ) VALUES (?,?,?)';
				   			$data = array($item[$i], $ep_id, $j+1);
	   			   //      	debug_msg("第".__LINE__."行 data ", $data);
	        	  //        die();
...
function listCLASS(){
	...
	$form->addElement('hidden','file','ctrlEP_talent');
...
function EP2class($class){
	...
	$talent=0;
	$sql = "select DISTINCT talent from concept_item_talent WHERE exam_paper_id = '$ep_id' ORDER BY talent ASC ";
	$result =$dbh->query($sql);
	while ($row=$result->fetchRow())
		{
		 $talent++;
		 $num[$talent]=$row['talent'];
		}
	if(isset($num[$talent]))
		{
//	debug_msg("第".__LINE__."行 InputArray ", $num);
		...
		for($i=1;$i<=$talent;$i++)
			{
			 $sql = "select item_sn from concept_item_talent WHERE exam_paper_id = '$ep_id' and talent = '$num[$i]'";
			 $result =$dbh->query($sql);
			 $s=0;
			 while ($row=$result->fetchRow())
				{

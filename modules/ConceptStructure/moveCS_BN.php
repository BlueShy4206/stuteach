...
//檢查新單元是否已經存在
	...
	$sn = 0;
	$sql = "SELECT max(item_sn) FROM concept_item ";
	$result = $dbh->query($sql);
	$item_sn = $result->fetchRow();
	$sn = $item_sn['max(item_sn)'];
	//print_r($item_sn);
	
	$sql = "select * from concept_item where cs_id = '$cs_id' order by item_sn";
	$result = $dbh->query($sql);    
	while ($data = $result->fetchRow()) {
	...
	//$sql = "SELECT concept_item_parameter.item_sn, concept_item_parameter.cs_id, concept_item_parameter.a, concept_item_parameter.b, concept_item_parameter.c, concept_item_parameter.tmp_N, concept_item_parameter.S_N, concept_item_parameter.A_N, concept_item_parameter.EP, concept_item_parameter.dim, concept_item_parameter.sub FROM concept_item,concept_item_parameter WHERE concept_item.item_sn =concept_item_parameter.item_sn and concept_item.cs_id='$cs_id' order by concept_item_parameter.item_sn";
	$sql = "SELECT concept_item_parameter.item_sn, concept_item_parameter.cs_id, concept_item_parameter.a, concept_item_parameter.b, concept_item_parameter.c, concept_item_parameter.tmp_N, concept_item_parameter.S_N, concept_item_parameter.A_N, concept_item_parameter.EP, concept_item_parameter.dim, concept_item_parameter.sub FROM concept_item,concept_item_parameter WHERE concept_item.item_sn =concept_item_parameter.item_sn and concept_item_parameter.cs_id='$cs_id' order by concept_item_parameter.item_sn";
	$result = $dbh->query($sql);    
	while ($data = $result->fetchRow()) {

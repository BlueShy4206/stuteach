...
//	echo sizeof($InputArray)."<br>";
			...
			if ( strlen($InputArray[$j][0]) && $InputArray[$j][0]!='') { //第一筆為抬頭，不檢查，ROW[0]：姓名不為空值
			 ...
			 //  $MaxArrayKey=max(array_keys($InputArray[$j]));
	            ...
	            for($i=1;$i<=1;$i++){
						...
						$ROW[$i]=trim($InputArray[$j][$i]);   //去除空格
				//		echo $ROW[$i];
						if (isset($ROW[$i]) && $j<(sizeof($InputArray)-1))
							{
							$roww.=$ROW[$i]._SPLIT_SYMBOL;
	        	  ...
	        	  //        die();
						...
						//debug_msg("第".__LINE__."行 data ", $roww);
					}
		...
		$sql = "UPDATE concept_info_dim SET sub_score_name = '$roww' WHERE cs_id = '$cs_id' ";
...
function EP2class($class){
	...
	$form->addElement('header','newheader','匯入能力指標名稱');
...
echo '能力指標結構說明：<br>';
echo '1.在Windows環境下，利用 excel 鍵入能力指標名稱，存成 excel 檔，如 <a href="examples/import_talent_name.xls">範例檔</a>。<br>';
//echo '2.在Linux下，鍵入帳號資料，存成 csv 檔，並保留第一列標題檔，如 <a href="examples/import_userALL.csv">範例檔</a>。<br>';
echo '2.利用本功能可自行設定能力指標名稱。<br>';
echo '</td>';

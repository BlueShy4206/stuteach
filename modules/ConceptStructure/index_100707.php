	...
	if ($form->validate() && $opt=='creat_cs') {
      ...
      //-- 寫入資料庫
		...
		if($result=="1"){
         ...
         if($result=="1"){
			   $Msg= "單元結構(".$_REQUEST['myunitname'].")建立成功！" ;
...
//            }
	...
	$form->addElement('header', 'myheader', '建立單元結構');  //標頭文字
   ...
   if($user_data->user_id=="admin"){
	...
	$form->addElement('text', 'myunitname', '單元名稱');
	$form->addElement('text', 'mysubnumber', '能力指標數量(數字)');
...
/*	$form->addElement('file','userfile[]','1.單元Rtree xls');
	...
	$form->addRule('myunitname', '單元名稱不可空白', 'required',null, 'client', null, null);
...
function modifyCS($mopt, $cs_sn){
	...
	if ($mopt=='modify') {   //第一次輸入資料要修改
      ...
      $upload = new HTTP_Upload();
		...
		foreach ($files as $file) {
			...
			if($this_upload_file[$i]==''){      //未上傳檔案，表示原檔案不更改
				$this_upload_file[$i]=$myCS->cs_file[$i];
			}else{
				$myCS->cs_file[$i]=strtolower($this_upload_file[$i]);
			}
			/*
      ...
      include_once "modules/$module_name/DbConceptRemedy.php";
        ...
        */
		...
		if($result){
			echo '<br>單元結構（'.$_REQUEST['myunitname'].'）修改成功！<br>';
		}
	}
	...
	$form1->addElement('header', 'myheader', '修改單元結構');  //標頭文字
	...
	$form1->addElement('text', 'myunitname', '單元名稱');
...
if($opt=='delete' && isset($_REQUEST['cs_sn'])){
	...
	if($cs_id){
		...
		$result = $dbh->query($sql);
		$sql="DELETE FROM exam_record WHERE cs_id='".$cs_id."'";
		$result = $dbh->query($sql);
...
<table width="100%" border="0" align="center">
  <tr>
	...
	<td align="right">【<a href="modules.php?op=modload&name=ConceptStructure&file=index">新增單元結構</a>】</td>
  </tr>
</table>

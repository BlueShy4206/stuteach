...
function editITEM($opt){
	...
	$ItemCR=$item_data->getEduParameter();
	...
	//debug_msg("第".__LINE__."行 sol_pieces ", $sol_pieces);
...
function updateITEM($item_sn){
   ...
   for($i=0;$i<$ii;$i++){
	...
	//debug_msg("第".__LINE__."行 op_filename ", $item_filename);
...
//die();
    ...
    //debug_msg("第".__LINE__."行 op_filename ", $result);                    
	...
	if($_REQUEST['parameter_a']!=0)
		$P_a=$_REQUEST['parameter_a'];
	if($_REQUEST['parameter_b']!=0)
		$P_b=$_REQUEST['parameter_b'];
	if($_REQUEST['parameter_c']!=0)
		$P_c=$_REQUEST['parameter_c'];
	if($_REQUEST['parameter_dim']!=0)
		$P_dim=$_REQUEST['parameter_dim'];
	if($_REQUEST['parameter_sub']!=0)
		$P_sub=$_REQUEST['parameter_sub'];
	...
	if($result==1){
		...
		//$RedirectTo='Location: modules.php?op=modload&name=ConceptStructure&file=modifyEP&opt=listEP&opt2=modifyEP&unit_item[0]='.$item->publisher_id.'&unit_item[1]='.$item->subject_id.'&unit_item[2]='.$item->vol.'&unit_item[3]='.$item->unit.'&unit_item[4]='.$item->paper_vol.'&ShowSol='.$ShowSol;
    $RedirectTo='modules.php?op=modload&name=ConceptStructure&file=modifyEP&opt=listEP&opt2=modifyEP&unit_item[0]='.$item->publisher_id.'&unit_item[1]='.$item->subject_id.'&unit_item[2]='.$item->vol.'&unit_item[3]='.$item->unit.'&unit_item[4]='.$item->paper_vol.'&ShowSol='.$ShowSol;
    unset($item);
...
function deleteITEM($opt){
	...
	if($opt=='deleteITEM' && isset($_REQUEST['item_sn'])){
		...
		$result = $dbh->query($sql);
		$sql="DELETE FROM concept_item_testlet WHERE item_sn='".$_REQUEST['item_sn']."'";
		$result = $dbh->query($sql);
	}
	//echo "<pre>";
	//print_r($_REQUEST);
	$RedirectTo='modules.php?op=modload&name=ConceptStructure&file=modifyEP&opt=listEP&opt2=modifyEP&unit_item[0]='.$_REQUEST['pid'].'&unit_item[1]='.$_REQUEST['sid'].'&unit_item[2]='.$_REQUEST['vid'].'&unit_item[3]='.$_REQUEST['uid'].'&unit_item[4]='.$_REQUEST['paper'];
  echo "<script language=\"javascript\">"; 
  echo "location.href='".$RedirectTo."'"; 
  echo "</script>";
}

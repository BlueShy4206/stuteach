...
//	echo $select_num;
	for($i=1;$i<=$n;$i++){
		$sqlstr="SELECT concept_item.item_num,concept_item.cs_id,concept_item.paper_vol,
	...
	if($_SESSION["selected_item"]){
		...
		while ($row = $result->fetchRow()) {
			...
			$item_length = $row[item_length]; //  題組測驗數量
	...
	for($i=1;$i<=$n;$i++){
		$sqlstr="SELECT concept_item.op_ans,concept_item_parameter.a
...
if($select_num == $tmp){
	...
	if($select_num <=$test_end){
		for($x=1;$x<=$n;$x++){
			...
			//debug_msg("第".__LINE__."行 _SESSION[select_item_sub_A] ", $_SESSION['select_item_sub_A']);
			$tableH=0;
	           ...
	           <tr>
	            <td scope="col"><table width="100%" border="0" cellpadding="2" cellspacing="0" class="title">
	              <tr>
	                ...
	                <td width="50%" scope="col" align="left">第'.$a.'題</td>';
	...
	//debug_msg("第".__LINE__."行 SESSION ", $_SESSION);

...
//判斷 F5
if(isset($_POST['user_answer'])&&isset($_SESSION['IRT'])&&$_SESSION['IRT']['code']==$_POST['code']){
  /*
...
//確認是否有中斷預存資料
if((!isset($_POST['user_answer']))&&(!isset($_SESSION["selected_item"]))){  //如果是第一次作答
...
$question=new Item_Structure4IRT($CsID, $IRT->select_item_num, $PaperVol); //呼叫試題圖檔

//出題
$tableH=0;
$question_select_pic=$question->get_item_select_pic();
echo '<br><table width="700" border="0" cellspacing="6" cellpadding="0">
     <tr><td><h1 align="right">'.$_SESSION['user_data']->user_id.'</td></tr>
     <tr>
      <td scope="col"><table width="100%" border="0" cellpadding="2" cellspacing="0" class="title">
        ...
        <tr>
          <td scope="col"><img src="'._THEME_IMG.'li.gif" width="11" height="28" /></td>
          		<td width="50%" scope="col" align="left">第'.$IRT->select_num.'題【'.$IRT->select_item_num.'】</td>';
...
if(isset($showfig)){		unset($showfig);	}
  ...
  $showfig[0]=str2compiler($showfig[0]);
  echo "<tr><td width=\"50\" align=\"center\" scope=\"col\"><input type=\"radio\" name=\"$the_user_answer\" value=\"".($i+1)."\" id=\"sq".$i."\"></td><td width=\"650\" align=\"left\"  class=\"s_title\" height=\"".$PImgProp['op_pieces'.$i][1]."\"><label for=\"sq".$i."\"><img border=\"0\" src=\"viewfig.php?list=".$showfig[0]."&tpp=".$showfig[1]."\"></label></td></tr>\n";
}
	echo "</table>";
echo '</td></tr></table>'; 
...
echo '<input name="screen" type="hidden" value="all">';
  echo "<center><input type=\"submit\" value=\"選擇完畢，進入下一題\" name=\"submit\"  class=\"butn01\" onClick=itemtimesent()>\n";
  echo '</form>';
  echo "<table>
        <center>
        <tr>
...
if(isset($_SESSION['time']['test_time']) and intval($_SESSION['time']['test_time'])>0){
	...
	$redirectURL=_ADP_URL."modules.php?op=modload&name=".$module_name."&file=".$SubmitFile."&screen=all&TestTimeIsUP=1&h_exam=".$h_exam."&cs_id=".$CsID."&CsID=".$CsID."&paper_vol=".$PaperVol."&select_num=".$select_num."&selected_item=".$selected_itemm."&item_sn=".$item_snn."&P_a=".$P_a."&P_b=".$P_b."&P_c=".$P_c."&exam_type=".$_SESSION["exam_type"];
...
function make_session_to_sql($user_id , $course_id ,$cs_id , $paper_vol , $post){
  ...
  //unset($session_temp[_authsession]);
  //$session_temp=array_merge($_SESSION['IRT'],$_SESSION['time']);
  
  $session_tmp=array($_SESSION['IRT'],$_SESSION['time']);  

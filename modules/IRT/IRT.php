...
//判斷是否繼續施測(以時間計算)
//$NowtimeStamp=time();
$NowtimeStamp=$_SESSION['time']['StartTimestamp']+round($_SESSION['time']['client_items_idle_time_total']/1000);
//考試時間已到，而且確實開始考試
//debug_msg("第".__LINE__."行 _SESSION['time'] ", $_SESSION['time']);
...
if(isset($_SESSION['time']['StopTimestamp']) ){
  if($_SESSION['time']["test_time_sec"]>0 and $_REQUEST['TestTimeIsUP']=='1'){   //確實有在考試
    ...
    //記錄client單題作答時間(毫秒)
    $client_item_idle_time=$_POST["client_end_ms"]-$_POST["client_start_ms"];    
    //debug_msg("第".__LINE__."行 IRT->step ", $IRT->step);
    //echo "client_end_ms=".$_POST["client_end_ms"]."<br>";   
    //echo "client_start_ms=".$_POST["client_start_ms"]."<br>"; 
    //echo "client_item_idle_time=".$client_item_idle_time."<br>";
    $_SESSION['time']["client_items_idle_time"].=$client_item_idle_time._SPLIT_SYMBOL;
    $_SESSION['time']['client_items_idle_time_total']=$_SESSION['time']['client_items_idle_time_total']+$client_item_idle_time;
    ...
    //debug_msg("第".__LINE__."行 cr_item ", $_POST['cr_item']);
    //echo "cr_item:".$_POST['cr_item']."<br>"; 
    //echo "cr_bin_res:".$_POST['cr_bin_res']."<br>";
    //echo "user_answer:".$_POST['user_answer']."<br>";
    //答題記錄
    if($IRT->step==1){  
      $IRT->pretest_rec_user_answer.=$_POST["user_answer"]._SPLIT_SYMBOL;
      $IRT->pretest_select_item_id.=$IRT->select_item_sn._SPLIT_SYMBOL;
      if($_POST['cr_item']=='1'){
        $IRT->pretest_response.=$_POST['cr_bin_res']._SPLIT_SYMBOL;
      }else{
    ...
    if($IRT->step==2){
      ...
      $IRT->h_exam=$IRT->get_EAP();
      $IRT->h_exam_A.=$IRT->h_exam._SPLIT_SYMBOL;
      $IRT->error_d = abs($IRT->h_exam-$IRT->h_exam_b);
    }
    //能力估計 end
...
if(isset($_POST['user_answer'])&&isset($_SESSION['IRT'])){
  ...
  if($IRT->step==1){  
    $IRT->pretest_rec_user_answer.=$_POST["user_answer"]._SPLIT_SYMBOL;
    $IRT->pretest_select_item_id.=$IRT->select_item_sn._SPLIT_SYMBOL; 
      if($_POST['cr_item']=='1'){ //add by coway 106.11.6
        $IRT->pretest_response.=$_POST['cr_bin_res']._SPLIT_SYMBOL;
      }else{
    if($IRT->get_item_op_ans($IRT->select_item_num)==$_POST["user_answer"]){
      $IRT->pretest_response.="1"._SPLIT_SYMBOL;
  ...
  if($IRT->step==1){
    if($IRT->max_sub_itemlength[$IRT->get_pretest_sub()-1]==$select_sub[$IRT->get_pretest_sub()-1]){
      ...
      if($pretest_percent>=0.8){
        $IRT->h_exam=1.5;
      }elseif($pretest_percent>=0.6){
        $IRT->h_exam=0.75;
      }elseif($pretest_percent>=0.4){
        $IRT->h_exam=0;
      }elseif($pretest_percent>=0.2){
        $IRT->h_exam=-0.75;  
      }else{
        $IRT->h_exam=-1.5;
      }
  ...
  if($IRT->step==1){
    ...
    //判斷是否在題組中
    //debug_msg("第".__LINE__."行 IRT->in_testlet ", $IRT->in_testlet);
    //debug_msg("第".__LINE__."行 IRT->testlet_info ", $IRT->testlet_info);
    if($IRT->in_testlet>0){
      $IRT->in_testlet++;
      if($IRT->testlet_info[$IRT->in_testlet]){
        $IRT->select_item_num=$IRT->testlet_info[$IRT->in_testlet]["item_num"];
        $IRT->select_item_sn=$IRT->testlet_info[$IRT->in_testlet]["item_sn"];
      }else{
        unset($IRT->in_testlet);
        unset($IRT->testlet_info);
        //決定要做哪一個 $sub
        $sub=$IRT->next_sub();    
  ...
  $_SESSION['time']['client_items_idle_time_total']=0;
  //......................................end
...
$sql = "select * from concept_item where cs_id = '$cs_id' and paper_vol = '".$paper_vol."' and item_num='$IRT->select_item_num' limit 0,1";
		
		$result = $dbh->query($sql);
		while ($data = $result->fetchRow()) {
			$item_filename=$data['item_filename'];
			$op_filename=$data['op_filename'];
			$cr_filename=$data['edu_parameter'];
		}
...
//debug_msg("第".__LINE__."行 IRT->step ", $IRT->step);
//debug_msg("第".__LINE__."行 IRT->h_exam_A ", $IRT->h_exam_A);
...
if($user_data->access_level==9){
	$show_select_item_num=$IRT->select_item_num;
}else{
	$show_select_item_num='';
}

echo '<br><table width="100%" border="0" cellspacing="6" cellpadding="0">
     ...
     <tr>
      <td scope="col"><table width="100%" border="0" cellpadding="2" cellspacing="0" class="title">
        ...
        <tr>
          		...
          		<td width="50%" scope="col" align="left"><font size="5" style="font-family:標楷體;">第'.$IRT->select_num.'題</font>'.$show_select_item_num.'</td>';
//'->'.$cr_filename;
...
echo '<table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr>
          ...
          <td><table width="100%" border="0" cellspacing="0" cellpadding="0" height="0">
            <tr>
              <td align="left" scope="col" height="0"><img name="Exam_Q" src="'.$_SESSION['pic_path'].$item_filename.'" border="0">';	
...
$op_pieces = explode(_SPLIT_SYMBOL, $op_filename);
//print_r($op_pieces);
//print_r(count($op_pieces));
	for($i=0;$i<count($op_pieces)-1;$i++)
	{
	if(isset($showfig)){		unset($showfig);	} 
	echo "<tr>
			<td width=\"50\" align=\"center\" scope=\"col\">
			<input type=\"radio\" name=\"$the_user_answer\" style=\"width:25px;height:25px;\" value=\"".($i+1)."\" id=\"sq".$i."\" required=\"required\"></td>
		<td width=\"650\" align=\"left\"  class=\"s_title\" height=\"0\">
		<label for=\"sq".$i."\">
		<img name=\"Exam_t".$i."\" border=\"0\" src=\"".$_SESSION['pic_path'].$op_pieces[$i]."\">
		</label></td></tr>\n";
}
	echo "</table>";
}else{   //建構題
	require_once 'ck_cr/'.$cr_filename;
	echo '<input name="user_answer" type="hidden" value="">';
  echo '<input name="cr_org_res" type="hidden" value="">';
  echo '<input name="cr_bin_res" type="hidden" value="">';
  echo '<input name="cr_item" type="hidden" value="1">';
	$cr_extra_function='document.keypad.user_answer.value="未記錄";';
  $cr_extra_function.='document.keypad.cr_org_res.value="未記錄";';
  $cr_extra_function.='document.keypad.cr_bin_res.value="未記錄";';
	$cr_extra_function.='submit();';
	
}


echo "<font size = \"6\" color=\"#a52a2a\" style=\"font-family:標楷體;\" >點選<image width=\"50\" height=\"37\" src=\"images/next_item.png\">，送出作答後，無法再修改！</font>"; //新加入的
echo '</td></tr></table>'; 
...
echo '<center><input type="image" value="進入下一題" name="submit" alt="Submit"  src="images/next_item.png" onClick="itemtimesent()">'."\n";
echo '</form>';
echo "<table>
        <center>
        <tr>
...
if($cr_filename==''){ //非建構題，才能把圖片變大變小	  
  ...
  //記錄本題考試時間
  $_SESSION['time']["item_start_time"]=time();
...
if(isset($_SESSION['time']['test_time']) and intval($_SESSION['time']['test_time'])>0){
	...
	$ItemTestTime=$_SESSION['time']['StopTimestamp']-time();//update by coway 2017.6.26           
  //$ItemTestTime=$_SESSION['time']['StopTimestamp']-($_SESSION['time']['StartTimestamp']+round($_SESSION['time']['client_items_idle_time_total']/1000));
  //echo "StartTimestamp=".$_SESSION['time']['StartTimestamp']."<br>";
  //echo "StopTimestamp=".$_SESSION['time']['StopTimestamp']."<br>";   
  //echo "ItemTestTime=".$ItemTestTime."<br>";
	echo '

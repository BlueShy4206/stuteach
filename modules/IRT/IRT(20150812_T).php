...
if((!isset($_POST['user_answer']))&&(!isset($_SESSION['IRT']))){  //如果是第一次作答
  ...
  $sql="SELECT * FROM session_data WHERE user_id='".$_POST['cs_id']."' AND course_id='".$_SESSION['AuthCourseID']."' AND cs_id='".$_POST['cs_id']."' AND paper_vol='".$_POST['paper_vol']."' ";
  $result = $dbh->query($sql);
  while ($data = $result->fetchRow()){
    $tmp=json_decode(gzuncompress(base64_decode($data['session_data'])),true);    
    $_SESSION['IRT']=$tmp[0];
...
echo '<br><table width="700" border="0" cellspacing="6" cellpadding="0">
     <tr><td><p align="right"><font size="7">'.$_SESSION['user_data']->user_id.'</p></td></tr>
     <tr>
      <td scope="col"><table width="100%" border="0" cellpadding="2" cellspacing="0" class="title">
        ...
        <tr>
          <td scope="col"><img src="'._THEME_IMG.'li.gif" width="11" height="28" /></td>
          		<td width="50%" scope="col" align="left"><font size="5">第'.$IRT->select_num.'題【'.$IRT->select_item_num.'】</font></td>
				<tr><td><input type="button" value="大" onclick="AutoResizeImage(0,0)">
				<input type="button" value="中" onclick="AutoResizeImage(900,0)">
				<input type="button" value="小" onclick="AutoResizeImage(700,0)">
				<input type="button" value="等比變大" onclick="AutoResizeImage2(2,2)">
				<input type="button" value="等比變小" onclick="AutoResizeImage2(0.5,0.5)">
				<input type="button" value="增長" onclick="AutoResizeImage2(3,118)">
				<input type="button" value="縮短" onclick="AutoResizeImage2(4,118)">
				</td></tr>';
...
echo "<input id=\"layoutsize\" type=\"text\" value=".$_post['layoutsize'].">";
  echo "<center><input type=\"submit\" value=\"選擇完畢\n進入下一題\" name=\"submit\"  class=\"butn01\" onClick=itemtimesent()>\n";
  echo '</form>';
  echo "<table>
        <center>
        <tr>
  ...
  echo "</div>
		...
		function AutoResizeImage(maxWidth,maxHeight){ 
			...
			document.getElementById(\"layoutsize\").value = maxWidth;
			
			var img = new Image(); 
			img.src = objImg.src; 
			//alert ('The image size is '+objImg_t1.width+'*'+objImg_t1.height);
			var hRatio; 
			var wRatio; 
			var Ratio = 1; 
			var w = objImg.width; 
			...
			//objImg.height = h;
			objImg.width = w; 
			//objImg_t1.height = objImg_t1.height*maxHeight;
			//objImg_t1.width = w_t;
			//objImg_t2.height = objImg_t2.height*maxHeight;
			//objImg_t2.width = w_t;
			//objImg_t3.height = objImg_t3.height*maxHeight;
			//objImg_t3.width = w_t;
			//objImg_t4.height = objImg_t4.height*maxHeight;
			//objImg_t4.width = w_t;
			//var width;
			//location.href=\"test.php?width=\"objImg.width;
			//alert(document.getElementById(\"layoutsize\").value = 700);
			
		}
		
		function AutoResizeImage2(maxWidth,maxHeight){ 
			var objImg = document.images[\"Exam_Q\"]; 
			var objImg_t1 = document.images[\"Exam_t0\"]; 
			var objImg_t2 = document.images[\"Exam_t1\"]; 
			var objImg_t3 = document.images[\"Exam_t2\"]; 
			...
			var img = new Image(); 
			img.src = objImg.src; 
			//alert ('The image size is '+objImg.width+'*'+objImg.height);
			
			 
			if(maxHeight > 110){
				if(maxWidth == 3)
				{
					objImg.height = objImg.height+maxHeight;
					//objImg.width = objImg.width*maxWidth;
					//objImg_t1.height = objImg_t1.height+50;
					//objImg_t2.height = objImg_t2.height+50;
					//objImg_t3.height = objImg_t3.height+50;
					//objImg_t4.height = objImg_t4.height+50;
				}else{
					objImg.height = objImg.height-maxHeight;
					//objImg_t1.height = objImg_t1.height-50;
					//objImg_t2.height = objImg_t2.height-50;
			...
			//objImg_t1.height = objImg_t1.height*maxHeight;
			//objImg_t1.width = objImg_t1.width*maxWidth;
			//objImg_t2.height = objImg_t2.height*maxHeight;
			//objImg_t2.width = objImg_t2.width*maxWidth;
			//objImg_t3.height = objImg_t3.height*maxHeight;
			//objImg_t3.width = objImg_t3.width*maxWidth;
			//objImg_t4.height = objImg_t4.height*maxHeight;
			//objImg_t4.width = objImg_t4.width*maxWidth;

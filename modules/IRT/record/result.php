<?php
  $IRT=unserialize($_SESSION['IRT']);
    ...
    */     
	...
	//---start 先檢查作答記錄中有沒有資料  105.11.06 by 彥鈞
	$repeat_data=0;
	$sql="SELECT user_id FROM exam_record_irt  WHERE exam_title='".$epid."' AND user_id='".$user_id."' AND course_id='".$_SESSION['AuthCourseID']."'";
	...
	//end of 先檢查作答記錄中有沒有資料  105.11.06 by 彥鈞

	if($repeat_data==0){    //確定資料庫中沒重複資料才寫入新資料  105.11.06 by 彥鈞   
  		//預試
    	...
    	if($result2==TRUE){
			...
			debug_msg("第".__LINE__."行 result2 ", $result2);

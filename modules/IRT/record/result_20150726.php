<?php
	...
	$total_items = $_SESSION["select_num"]-$_SESSION["select_num_temp"];

	$exam_type_all = $_SESSION['exam_type']._SPLIT_SYMBOL.$type_id_t._SPLIT_SYMBOL.$type_id_s._SPLIT_SYMBOL.$type_id_e._SPLIT_SYMBOL.$num_e._SPLIT_SYMBOL.$item_length;
	//echo $MIRT_d.'<br>';
	//echo $CsID.'<br>';  
	echo "<br>恭喜你測驗完畢!<P>";
	if ($MIRT_d>1)
	{
		$h_exam_tmp= explode(_SPLIT_SYMBOL,$_SESSION["h_exam_A"]);//暫存能力估計值陣列
		echo '<table border="1">
            <tr>
              <td>向度</td>                    
              <td>能力估計值</td>
      ...
      //...................................................................計算各能力向度的百比分數 end
		...
		mysql_free_result($sql);
		$concept = explode(_SPLIT_SYMBOL,$sub_score_name);
		$unit = count($concept);
		//  end
		//  計算每一個次級量尺有幾題 
		//............................................................function_P    1
		...
		$Siia = explode(_SPLIT_SYMBOL,$_SESSION['select_item_id_A']);
		//debug_msg("第".__LINE__."行 _SESSION ", $_SESSION);
		//debug_msg("第".__LINE__."行 Siia ", $Siia);
		for($i=0;$i<$total_items;$i++)
		{
			$Siia_i .= $Siia[$i]."','";
		}
		$sqlstr="SELECT `a`,`b`,`c`,`sub` FROM `concept_item_parameter`
		...
		for($i=0;$i<$unit;$i++)
		{
			...
			$sub_score[$i] = $tmp/$n;
			//debug_msg("第".__LINE__."行 sub_score ", $sub_score);
...
*/        
      ...
      echo '<table border="1">
            <tr>
              ...
              <td><div align="center">量尺分數</div></td>
            </tr>';
      for($i=0;$i<$unit;$i++)
      {
        echo '<tr>
    ...
    //-- 寫入資料庫
      ...
      {
        $select_item_id_S_new1 .= $select_item_id_S_new[$i]._SPLIT_SYMBOL;
      }
      ...
      {
        $select_item_id_A_new1 .= $select_item_id_A_new[$i]._SPLIT_SYMBOL;
      }
    ...
    //...................................................................紀錄學習紀錄   
    $EpID=$CsID.sprintf("%02d",$_SESSION["PaperVol"]);
    $sql = 'INSERT exam_record_irt (user_id, cs_id, date, start_time, stop_time, during_time, firm_id,            
                                    ...
                                    exam_res, sub_score_name, sub_score_res, paper_vol, type_id, theta_res, dim, course_id, exam_title)
                            ...
                            '","'.$exam_type_all.'","'.$_SESSION["select_item_b_theta"].'","'.$MIRT_d.'","'.$_SESSION[AuthCourseID].'","'.$EpID.'")';
    //debug_msg("第".__LINE__."行 _SESSION ", $_SESSION);
	//die();

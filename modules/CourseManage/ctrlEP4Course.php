...
if($_REQUEST['opt']=='EP2course'){
	...
	EP2course($_REQUEST['course'],$_REQUEST['mysubject']);

}elseif($_REQUEST['opt']=='EPadd2Course' && count($_REQUEST['EPids'])>0){
	EPadd2course($_REQUEST['course'], $_REQUEST[EPids], $_REQUEST[ExamType], $_REQUEST[TestTime], $_REQUEST['ExamTRangeS'], $_REQUEST['ExamTRangeE']);
...
function listCourse(){
	...
	//-- 顯示選單
	//echo "☆★☆ 課程列表 ☆★☆<br>";
	$form->addElement('header', 'myheader', '<center> 場次與試卷存取控制 </canter>'); 
	// Create the Element
	...
	$form->addElement('select', 'mysubject', '請選擇領域：', $subject);
	$form->addElement('submit','btnSubmit','選擇完畢，送出！');
	$form->display();
}


function EP2course($course ,$mysubject){
	global $dbh, $module_name, $SubmitFile;
	$CO=new CourseData($course[1]);
	$subject = $mysubject;//teresa add 領域條件
	//debug_msg(__LINE__."行  ep", $ep);

	//$listCOURSE='<a href="modules.php?op=modload&name='.$module_name.'&file='.$SubmitFile.'">選擇其他課程</a>';
	echo '<br>現在的場次是：<font color=red><b>'.$CO->FullName.'('.$subject.'領域)</b></font><hr>';
	...
	while ($data = $result->fetchRow()) {
		...
		$AllCourseName[$data['course_id']]=$data[year]."學年度-".$data[name];
		$IDcount++;
	}
	...
	echo '<TABLE BORDER="1" CELLPADDING="1" CELLSPACING="1" WIDTH="100%">
        ...
        */          
	        ...
	        /*   echo '<tr><td align="center">';
             ...
             echo '<td align="center"><input type="text" name="ExamType[item_l]" size=2 value="0">';  //題組試題數量
             echo '</td></tr></table>'; 
        ...
        */
        echo '<INPUT TYPE="HIDDEN" NAME="course[0]" VALUE="'.$course[0].'">';
               ...
               echo '<INPUT TYPE="HIDDEN" NAME="ExamType[irt]" VALUE="1">';
               echo '<INPUT TYPE="HIDDEN" NAME="ExamType[t]" VALUE="2">';
               echo '<INPUT TYPE="HIDDEN" NAME="ExamType[s]" VALUE="1">';
               echo '<INPUT TYPE="HIDDEN" NAME="ExamType[e]" VALUE="1">';
               echo '<INPUT TYPE="HIDDEN" NAME="ExamType[num_e]" VALUE="0">';
               echo '<INPUT TYPE="HIDDEN" NAME="ExamType[item_l]" VALUE="0">';
		echo '
      <tr>
        <td bgcolor="#FFFFCC" align="center" WIDTH="10%">
          <font size="3"><b>無試題參數題目</b></font>
        <td bgcolor="#FFFFCC" VALIGN="TOP" align="center">
          <label> <input type="radio" name="ExamType[noABC]" value="1" >前五題出無試題參數題目 </label> <br>
          <label> <input type="radio" name="ExamType[noABC]" value="0" >不要出無試題參數題目 </label>
    ';
    echo '
      <tr>
        <td bgcolor="#FFFFCC" align="center" WIDTH="10%">
          <font size="3"><b>題組題目</b></font>
        <td bgcolor="#FFFFCC" VALIGN="TOP" align="center">
          <label> <input type="radio" name="ExamType[testlet]" value="1" >出兩組題組 </label> <br>
          <label> <input type="radio" name="ExamType[testlet]" value="0" >不要出出兩組題組 </label>
    ';
    	echo '<tr>
              <td bgcolor="#FFFFCC" align="center" WIDTH="10%">
		...
		echo '<tr>
              <td bgcolor="#FFFFCC" align="center" WIDTH="10%">
               <font size="3"><b>評量領域/時間區間</b></font>
              </td>  ';
		echo '<td bgcolor="#FFFFCC" VALIGN="TOP" align="center"><br>
                <font size="3" face="arial">';
		echo '<input name="ExamTRangeS" type="text" id="ExamTRangeS" onclick="displayCalendar(ExamTRangeS,\'yyyy-mm-dd hh:ii:00\',this,true)" /> 至 <input name="ExamTRangeE" type="text" id="ExamTRangeE"  onclick="displayCalendar(ExamTRangeE,\'yyyy-mm-dd hh:ii:00\',this,true)" /> 
          	<br>格式為：YYYY-MM-DD HH:II:SS</td>
		      </tr>';
        echo '<tr>
              <td bgcolor="#FFFFCC" align="center" WIDTH="10%">
              ...
              <td bgcolor="#FFFFCC" VALIGN="TOP" align="center">
                ...
                $sql="SELECT sn, exam_paper_id, type_id_irt, type_id_e, type_id_s, type_id_t, num_e, item_length, test_time FROM exam_course_access_irt WHERE year='".$course[0]."' AND course_id='".$course[1]."' ORDER BY exam_paper_id";
	              $result = $dbh->query($sql);
	              while ($data = $result->fetchRow()) {
	              ...
	              $sql = "SELECT distinct a.exam_paper_id FROM concept_item a, concept_info b WHERE a.cs_id = b.cs_id and (a.exam_paper_id != -1) and b.subject_id='".$subject."'";
	             ...
	             while ($data = $result->fetchRow())	{
		            ...
		            $paper_title=EPid2ShortName($data['exam_paper_id']);
       ...
       echo '<td bgcolor="#FFFFCC" VALIGN="TOP" align="center">
              ...
              $ii=sizeof($current_EP);
	           for($i=0;$i<$ii;$i++){
					...
					$item_l=substr($current_EP[$i],23,2);   //取出題組試題數量
					...
					$paper_title=EPid2ShortName($currEPid).'<br>';
...
function EPadd2course($CourseID, $EPids, $ExamType, $TestTime, $ExamTRangeS, $ExamTRangeE){
	global $dbh, $module_name, $SubmitFile;
	...
	if($con!=$i){   
		...
		while(list($null, $curr_EPid) = each($EPids)) {
			...
			$query = '
        INSERT INTO exam_course_access_irt 
        (cs_id, paper_vol, exam_paper_id, course_id, year, seme, type_id_irt, type_id_e, type_id_s, type_id_t, num_e, item_length, test_time, noABC, testlet, start_time, end_time) 
        VALUES 
        (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
      ';
			$data = array($cs_id, $paper_vol, $curr_EPid, $CourseID[1], $CourseID[0], '0', $ExamType['irt'], $ExamType['e'], $ExamType['s'], $ExamType['t'], $ExamType['num_e'], $ExamType['item_l'], $TestTime, $ExamType[noABC], $ExamType[testlet], $ExamTRangeS, $ExamTRangeE);

...
Class Diag_Report_IRT{
	...
	function Diag_Report_IRT($input_student,$input_concept){
		...
		$this->student_basic_data['idn']=$this->stu_data->identity;
      	...
      	$this->concept_title=$this->Concept_Object->cs_name;
               ...
               <tr>
                  ...
                  <tr>
                    <td><table width="100%" border="0" cellpadding="0" cellspacing="0">
                      <tr>
                        <td width="20%" height="35" align="center" background="'._THEME_IMG.'tit_bg03.gif" class="d_s_title01" scope="col">姓名(中文)</td>
                        <td width="30%" align="left" class="d_s_title02" scope="col">　'.$this->student_basic_data['學生姓名'].'</td>
                        <td width="20%" height="35" align="center" background="'._THEME_IMG.'tit_bg03.gif" class="d_s_title02" scope="col">姓名(英文)</td>
                        <td width="30%" align="left" class="d_s_title03" scope="col">　</td>
                      </tr>
                      <tr>
                        <td width="20%" height="35" align="center" background="'._THEME_IMG.'tit_bg03.gif" class="d_s_title04" scope="col">帳號</td>
                        <td align="left" class="d_s_title05">　'.$this->student_basic_data['sn'].'</td>
                        <td width="20%" height="35" align="center" background="'._THEME_IMG.'tit_bg03.gif" class="d_s_title05" scope="col">性別</td>
                        <td align="left" class="d_s_title06">　'.$this->student_basic_data['學生性別'].'</td>
                      </tr>
                      <tr>
                        <td width="20%" height="35" align="center" background="'._THEME_IMG.'tit_bg03.gif" class="d_s_title07" scope="col">身分證字號</td>
                        <td align="left" class="d_s_title08">　'.$this->student_basic_data['idn'].'</td>
                        <td width="20%" height="35" align="center" background="'._THEME_IMG.'tit_bg03.gif" class="d_s_title08" scope="col">教師證號碼</td>
                        <td align="left" class="d_s_title09">　</td>
                      </tr>
                    </table></td>  
                  </tr>
                </table></td>
    ...
    if($dim_s==1)
    {
		$print_data='<tr>
               ...
               <tr>
                  ...
                  <tr>
                    <td align="left" scope="col" bgcolor="#9BD8E7"><span class="d_title"> 測 驗 成 績 </span></td>
                  </tr>
                  <tr>
      ...
      <!--    <td width="12%" height="35" align="center" background="'._THEME_IMG.'tit_bg03.gif" class="d_s_title31" scope="col">百分等級</td>  -->
                          <td width="12%" height="35" align="center" background="'._THEME_IMG.'tit_bg03.gif" class="d_s_title32" scope="col">表現層級</td>
                        </tr>';
		...
		//	$theta_s = round($theta_l,4);
			...
			if($this->concept_title == "國文"){
			if(theta_turn_percentage($q_theta[$i]) > 603)
			$print_data.='<td align="center" class="d_s_title32">'.$this->done_items[$i].""."精熟"."</td>\n";
			if(theta_turn_percentage($q_theta[$i]) > 413 && theta_turn_percentage($q_theta[$i]) <= 603)
			$print_data.='<td align="center" class="d_s_title32">'.$this->done_items[$i].""."基礎"."</td>\n";
			if(theta_turn_percentage($q_theta[$i]) <= 413)
			$print_data.='<td align="center" class="d_s_title32">'.$this->done_items[$i].""."待加強"."</td>\n";
			}elseif($this->concept_title == "社會"){
			if(theta_turn_percentage($q_theta[$i]) > 606)
			$print_data.='<td align="center" class="d_s_title32">'.$this->done_items[$i].""."精熟"."</td>\n";
			if(theta_turn_percentage($q_theta[$i]) > 440 && theta_turn_percentage($q_theta[$i]) <= 606)
			$print_data.='<td align="center" class="d_s_title32">'.$this->done_items[$i].""."基礎"."</td>\n";
			if(theta_turn_percentage($q_theta[$i]) <= 440)
			$print_data.='<td align="center" class="d_s_title32">'.$this->done_items[$i].""."待加強"."</td>\n";
			}elseif($this->concept_title == "數學"){
			if(theta_turn_percentage($q_theta[$i]) > 608)
			$print_data.='<td align="center" class="d_s_title32">'.$this->done_items[$i].""."精熟"."</td>\n";
			if(theta_turn_percentage($q_theta[$i]) > 420 && theta_turn_percentage($q_theta[$i]) <= 608)
			$print_data.='<td align="center" class="d_s_title32">'.$this->done_items[$i].""."基礎"."</td>\n";
			if(theta_turn_percentage($q_theta[$i]) <= 420)
			$print_data.='<td align="center" class="d_s_title32">'.$this->done_items[$i].""."待加強"."</td>\n";
			}elseif($this->concept_title == "自然"){
			if(theta_turn_percentage($q_theta[$i]) > 587)
			$print_data.='<td align="center" class="d_s_title32">'.$this->done_items[$i].""."精熟"."</td>\n";
			if(theta_turn_percentage($q_theta[$i]) > 417 && theta_turn_percentage($q_theta[$i]) <= 587)
			$print_data.='<td align="center" class="d_s_title32">'.$this->done_items[$i].""."基礎"."</td>\n";
			if(theta_turn_percentage($q_theta[$i]) <= 417)
			$print_data.='<td align="center" class="d_s_title32">'.$this->done_items[$i].""."待加強"."</td>\n";
			}else{
			if(theta_turn_percentage($q_theta[$i]) > 606)
			$print_data.='<td align="center" class="d_s_title32">'.$this->done_items[$i].""."精熟"."</td>\n";
  ...
  //function print_remedy_data(){
	 $sql=mysql_query("SELECT `sub_score_name`, `sub_score_res`,`dim`,`paper_vol`,`select_item_id_S` ,`theta`  FROM `exam_record_irt`
    ...
    {  
	  ...
	  $select_item_id_S = $row[4];
	  $theta = $row[5];
    }
    mysql_free_result($sql);    
    
	$i_select_item_id_S=explode("@XX@",$select_item_id_S);
	$xz=0;
	$p_theta=0;
	while($xz < count($i_select_item_id_S)){
		$xz++;
			
	$sql=mysql_query("SELECT `a`, `b`,`c`,`sub`  FROM `concept_item_parameter`
                     WHERE `item_sn` like '".$i_select_item_id_S[$xz]."'  order by item_sn desc ");
	$xk=0;    
    while($row=mysql_fetch_array($sql))
    {  
	  $xk++;                   
      $concept_item_a = $row[0];
      $concept_item_b = $row[1];
      $concept_item_c = $row[2];
	  $concept_item_sub = $row[3];
	  
    }
    mysql_free_result($sql);    
	
	
	$p_theta=$concept_item_c+(1-$concept_item_c)*(exp(1.7*$concept_item_a*($theta-$concept_item_b)))/(1+(exp(1.7*$concept_item_a*($theta-$concept_item_b))));
	
	$p_theta1[$concept_item_sub-1] = $p_theta1[$concept_item_sub-1]+$p_theta;
	$p_theta1_count[$concept_item_sub-1]++;
	}
	
	//$p_theta_s= $p_theta1 / count($i_select_item_id_S);
    ...
    $print_data='<tr>                 
                 ...
                 <tr>
                  ...
                  <td>
                   ...
                   <table width="100%" border="0" cellspacing="0" cellpadding="0">
                     ...
                     <tr>                       
                       <td align="left" class="d_title" scope="col" bgcolor="#9BD8E7"> 次 領 域 報 告 </td>
                     </tr>
				             <tr>
                       <!--<td align="left" scope="col" bgcolor="#9BD8E7">說明：◎表示<font color="0000ff">通過</font>該概念；Ｘ表示<font color="ff0000">未通過</font>該概念。
        				       </td>-->
                     ...
                     <tr>
                       <td>
                         ...
                         <table width="100%" align="center" border="0" cellpadding="4" cellspacing="0" class="d_tableline">
                           <tr>';
		           $print_data.='<td width="45%" align="center" background="'._THEME_IMG.'tit_bg03.gif" class="d_s_title41">主題</td>';
               $print_data.='<td '.$tmp.' colspan='.$xj.' align="center" background="'._THEME_IMG.'tit_bg03.gif" class="d_s_title41">通過率</td>';
			   //<td '.$tmp.'  align="center" background="'._THEME_IMG.'tit_bg03.gif" class="d_s_title41">補救教學</td>'."\n";
		           $tmp="";
               if($this->test_times>1)
                   {
                 //$tmp='colspan="'.$this->test_times.'"';
             ...
             $print_data.="<tr>\n";
             $num = $i+1;
               $print_data.='<td '.$tmp.' align="center" background="'._THEME_IMG.'tit_bg03.gif" class="d_s_title41">&nbsp&nbsp'.$sub_score_name[$i].'</td>'."\n";
               $xi=$xj;
               while($xi>=1)
               {
				if($p_theta1[$i]==""){   
                $print_data.='<td '.$tmp.'  align="center" background="'._THEME_IMG.'tit_bg03.gif" class="d_s_title41">0</td>';}else{
				$print_data.='<td '.$tmp.'  align="center" background="'._THEME_IMG.'tit_bg03.gif" class="d_s_title41">'.number_format($p_theta1[$i]/$p_theta1_count[$i],4).'</td>';	
					}
				$xi--;   
   			   }
